...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  // Create user first to get userId for avatar upload
  ...
  if (avatarFile && avatarFile.size > 0) {
    avatarUrl = await uploadAvatar(avatarFile, createdUser.id) || undefined;
    
    // Update user with avatar URL if upload was successful
    if (avatarUrl) {
      await db
        .update(users)
        .set({ avatarUrl })
